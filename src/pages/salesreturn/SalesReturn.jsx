...
const SalesReturn = () => {
  ...
  const exportData = useMemo(() => {
    return billData && billData.map(({ returnData, salesReturnNumber, party }) => ({
      "Return Date": returnData,
      "Sales Invoice Number": salesReturnNumber,
      "Party": party.name,
    }));
  }, [billData]);
  ...
  const exportTable = async (whichType) => {
    if (whichType === "copy") {
      ...
      downloadExcel(exportData, 'sales-return.xlsx') // Pass data and filename
    }
    else if (whichType === "print") {
      printTable(tableRef, "Sales Return List"); // Pass table ref and title
    }
    else if (whichType === "pdf") {
      let document = exportPdf('Sales Return List', exportData);
  ...
  const removeData = async (trash) => {
    ...
    const url = process.env.REACT_APP_API_URL + "/salesreturn/delete";
    try {
      const req = await fetch(url, {
        method: "DELETE",
        headers: {
  ...
  const restoreData = async () => {
    ...
    const url = process.env.REACT_APP_API_URL + "/salesreturn/restore";
    try {
      const req = await fetch(url, {
        method: "POST",
        headers: {
  ...
  const getFilterData = async () => {
    ...
    try {
      const url = process.env.REACT_APP_API_URL + `/salesreturn/filter?page=${activePage}&limit=${dataLimit}`;
        ...
        <div className='content__body'>
          ...
          </div>
          {
            !loading ? billData.length > 0 ? <div className='content__body__main'>
              ...
              : <AddNew title={"Sales Return"} link={"/admin/sales-return/add"} />
              : <DataShimmer />
          }
        </div>
      </main>
