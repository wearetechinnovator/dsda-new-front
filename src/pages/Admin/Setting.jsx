...
const Setting = () => {
	...
	const generateAmenities = async () => {
		try {
			...
			const url = `${process.env.REACT_APP_MASTER_API}/amenities/amenities-sync`
			const req = await fetch(url, {
				method: "POST",
				headers: {
					"Content-Type": 'application/json'
			...
			return toast("Amenities generate successfully", 'success');
				...
				<div className='content__body'>
					<div className='content__body__main'>
						<div className='w-full flex gap-3 items-center'>
							...
							<button
								onClick={generateAmenities}
								disabled={amenityLoading}
								className='px-2 py-1 rounded bg-blue-500 hover:bg-blue-600 text-white'
							>
								{amenityLoading ? <Loading /> : <Icons.RUPES className='inline' />}Generate Amenities
							</button>
						</div>
