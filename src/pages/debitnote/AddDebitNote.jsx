...
const DebitNote = ({ mode }) => {
  ...
  useEffect(() => {
    if (mode) {
      const get = async () => {
        try {
          const url = `${process.env.REACT_APP_API_URL}/debitnote/get`;
  ...
  */
  ...
  const saveBill = async () => {
    ...
    try {
      ...
      if (mode === "edit") {
        return toast('Debit Note update successfully', 'success');
      }
  ...
  return (
    <>
      <Nav title={mode == "edit" ? "Update Debit Note" : "Add Debit Note"} />
      <main id='main'>
        <SideNav />
          ...
          <div className='content__body__main bg-white' id='addQuotationTable'>
            ...
            <div className='flex flex-col lg:flex-row items-center justify-around gap-4'>
              ...
              <div className='flex flex-col gap-2 w-full lg:w-1/3'>
                <p className='text-xs'>Debit Note Number <span className='required__text'>*</span></p>
                <input type="text"
              ...
              <div className='flex flex-col gap-2 w-full lg:w-1/3'>
                ...
                <input type="date"
                  className='text-xs'
                  onChange={(e) => {
                    setFormData({ ...formData, debitNoteDate: e.target.value })
                  }}
